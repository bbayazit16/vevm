...
Definition interpret (instr : Instruction) (state: VmState) : VmResult :=
    match instr with
        | IPush n => Ok {| stack := n :: state.(stack) ; pc := state.(pc) + 1; memory := state.(memory) |}
        | IMstore => match state.(stack) with
            | offset :: value :: s' => Ok {|
                stack := s' ; pc := state.(pc) + 1; memory := (NatMap.add offset value state.(memory))
            |}
            | _ => Err "mstore failed"
            end
        | IMload => match state.(stack) with
            | offset :: s' =>
                match NatMap.find offset state.(memory) with
                    | Some found => Ok {| stack := found :: s'; pc := state.(pc) + 1; memory := state.(memory) |}
                    | None => Err "failed to find key"
                end
            | _ => Err "failed to find key"
            end
        | IAdd => match state.(stack) with
            | a :: b :: s' => Ok {| stack := (a + b) :: s' ; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "add failed"
            end
        | IOutput => match state.(stack) with
            | a :: rest' => Ok {| stack := [a] ; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "output failed"
            end
        | IEq => match state.(stack) with
            | a :: b :: s' => Ok {| stack := (if Nat.eqb a b then 1 else 0) :: s' ; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "eq failed"
            end
        | IJmpi => match state.(stack) with
            | should_jump :: address :: s' => if Nat.eqb should_jump 1 then
                Ok {|stack := s'; pc := address; memory := state.(memory) |}
                else Ok {| stack := s'; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "jmp failed"
            end
        | IDup => match state.(stack) with
            | a :: s' => Ok {| stack := (a :: (a :: s')) ; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "dup failed"
            end
        | IPop => match state.(stack) with
            | a :: s' => Ok {| stack := s'; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "nothing to pop"
            end
        | ISwap => match state.(stack) with
            | a :: b :: s' => Ok {| stack := (b :: (a :: s')) ; pc := state.(pc) + 1; memory := state.(memory) |}
            | _ => Err "swap failed"
            end
end.
